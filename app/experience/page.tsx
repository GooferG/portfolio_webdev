      ...
      <FadeIn delay={index * 0.08}>
        ...
        >
          ...
          >
            ...
            >
              ...
              <span className="text-fg-subtle">{entry.date}</span>
            </div>

            {/* Title + Company */}
            <h2 className="text-fg-strong text-2xl md:text-3xl font-extrabold leading-tight mb-1">
              {entry.title}
            </h2>
            <p className="text-fg-subtle italic text-sm md:text-base mb-5">
              {entry.company} - {entry.location}
            </p>

            {/* Description */}
            <p
              className={`text-fg-muted leading-relaxed text-sm md:text-[15px] mb-5 ${
                isLeft ? 'md:ml-auto' : ''
              }`}
            >
              {entry.description}
            ...
            {entry.stack && entry.stack.length > 0 && (
              ...
              >
                {entry.stack.map((s) => (
                  <span
                    key={s}
                    className="px-2 py-0.5 text-xs bg-bg-card border border-border-subtle rounded text-fg-muted"
                  >
                    {s}
                  </span>
                ))}
        ...
        <span
          className={`counter-num absolute top-0 flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full bg-bg-card border-2 ${cat.dot.replace(
            'bg-',
            'border-'
          )} text-fg-strong text-sm md:text-base font-bold ring-4 ring-bg-primary left-0 md:left-1/2 md:-translate-x-1/2`}
          aria-hidden="true"
        />
      </FadeIn>
    </li>
